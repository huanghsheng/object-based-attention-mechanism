...
def compute_label_info(content_segment, style_segment):
    ...
    style_segment = torch.from_numpy(style_segment).cuda(0)
    content_segment = torch.from_numpy(content_segment).cuda(0)
    label_set = style_segment.unique().cpu().numpy()
    label_indicator = np.zeros(style_segment.max().item()+1)

    for l in style_segment.unique():
        if (style_segment==l).sum()>=effect_thresh and (content_segment==l).sum()>=effect_thresh:
            label_indicator[l] = 1
        print(l)
    #
    #
    ...
    #     if c_size > 100 and s_size > 100 and c_size / s_size < 100 and s_size / c_size < 100:
