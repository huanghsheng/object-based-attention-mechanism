...
def wct_core(cont_feat, styl_feat, cont_feat_view, content_index, style_index, weight=1, registers=None, device='cpu'):
    ...
    #
    # return targetFeature, target_shift
...
def wct_core_segment(content_feat, style_feat, content_segment, style_segment,
    ...
    for label in label_set:
        ...
        if content_index.size(0)>max_num:
            max_label = label
            max_num = content_index.size(0)
    ...
    # num_list = []
    #
    # cont_all_index = torch.zeros(content_feat.size(-2)*content_feat.size(-1)).to(device)
    ...
    if 2>1:
        ...
        _target_feature = wct_core(masked_content_feat, masked_style_feat, content_feat_view, content_index, style_index, weight, registers, device=device)
        target_feature = _target_feature.clone()

        # target_feature.index_copy_(1, content_index, _target_feature)
        #
        # cont_all_index[content_index] = 1
        # shift_list.append(_target_shift)
        # num_list.append(content_index.size(0))

    # num_total = float(np.array(num_list).sum())
    # aver_shift = torch.zeros(content_feat.size(1)).to(device)
    # for epoch,num in enumerate(num_list):
    #     aver_shift = aver_shift+(num/num_total)*shift_list[epoch]
    #     print(epoch,num)
    # cont_vers_index = torch.where(cont_all_index == 0)[0]
    # unTargetFeature = torch.index_select(content_feat_view, 1, cont_vers_index.cuda(0))
    # unTargetFeature = unTargetFeature + aver_shift.unsqueeze(1).expand_as(unTargetFeature)
