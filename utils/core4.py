...
def wct_core_segment(content_feat, style_feat, content_segment, style_segment,
    ...
    for label in label_set:
        ...
        # if not label_indicator[label]:
        #     continue
        if not ((resized_content_segment==label).sum()>resized_content_segment.size*mininum_rate and \
            (resized_style_segment==label).sum()>resized_style_segment.size*mininum_rate):
        ...
        target_feature.index_copy_(1, content_index, _target_feature)
        # target_feature[:, content_index] = _target_feature.clone()
        # break
